...
class SpindleDatasetBatcher(object):
  ...
  def parser(self, serialized_example):
    ...
    subject_std = {}
    subject_std['200'] = [13.8275,14.9942,15.5654,12.9483,12.2577,14.8564,17.8164,19.8062,17.2921,
                       19.2211,14.5055,19.9593,15.3751,16.2978,14.1444,23.005,15.7555,17.8625,11.5192]
    subject_std['50'] = [13.9342, 15.1944,15.946,13.0445,12.4503,15.1196,17.9145,20.0297,17.4839,
                        19.606,14.6044,20.0877,15.4103,16.3854,14.1842,23.3035,15.9078,18.0474,11.6242]
    subject_std['34'] = [13.8587, 15.0691, 15.6965, 12.9845, 12.3263, 14.9442, 17.8637, 19.8735, 17.3554,
                       19.3543, 14.5438, 20.014, 15.3866, 16.338, 14.167, 23.1198, 15.815, 17.9347, 11.5594]
    subject_std['100'] = [13.9789, 15.26, 16.1699, 13.0276, 12.4902, 15.2348, 17.8223, 20.0905, 17.5904, 19.7739, 14.631,
                         20.0389, 15.391, 16.3568, 14.2044, 23.3718, 15.9692, 18.0525, 11.6305]
    ...
    for k in range(self.num_feats):
      if k==0:
          if self.subset in ['test']:
            data1.append((data[:,k]-mean[k])/np.asarray(subject_std[str(self.target_f)])[int(self.test_sub[0])-1])  # MASS: 15.8103  Dream: 17.0197 MASS-sub1-or: 13.8275 MASS-sub1-and: 13.7274  MASS-sub18-and: 16.8832 MASS-sub18-or: 17.8448 Rats 2.0521e-4
          else:
            data1.append((data[:,k]-mean[k])/np.asarray(subject_std[str(self.target_f)])[np.asarray(self.subject,dtype=int)-1].mean())
      else:
        data1.append(data[:,k])
